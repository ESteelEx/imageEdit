...
"""imageEdit.py: Solves transparency issues regarding UI images for ModuleWorks SampleIntegration.
                 ...
                 replace them with 255 0 255, so that SI can take out this RGB values and replace them with UI background
                 color. All the ugly purple artifacts disappear. Everybody says WOOHOOO! """

__author__      = "Mathias Rohler"
__copyright__   = "Copyright 2015, ModuleWorks"

import wx, os
# What to do that import of PIL works, when you get the error message that module wasnt found.
...
def imageCorrection(image_directory, list_file_name):
    ...
    for imagename in list_file_name:
        ...
        if not os.path.exists(image_directory + '\\adjusted_images'):
            ...
            imagebgr.save(image_directory + '\\adjusted_images\\' + imagename)
        else:
            imagebgr.save(image_directory + '\\adjusted_images\\' + imagename)
